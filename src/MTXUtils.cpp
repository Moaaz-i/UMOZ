...
bool MTXUtils::applyHysteresis(float value, float threshold, float margin, bool currentOutputState) {
  if (currentOutputState) {
    if (value < (threshold - margin)) return false;
  } else {
    if (value > (threshold + margin)) return true;
  }
  return currentOutputState;
}

uint8_t MTXUtils::calculateBatteryPercentage(uint8_t analogPin, float r1, float r2, float minVolts, float maxVolts, float vRef, int adcResolution) {
