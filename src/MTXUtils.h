...
class MTXUtils {
  public:
    ...
    static uint8_t calculateBatteryPercentage(uint8_t analogPin, float r1, float r2, float minVolts, float maxVolts, float vRef = 5.0, int adcResolution = 1023);
    ...
    static inline bool hasTimeout(uint32_t startTime, uint32_t interval) {
    ...
    static inline uint8_t getCenterOffset(uint8_t lcdWidth, const char* text) {
      ...
      if (len >= lcdWidth) return 0;
      return (lcdWidth - len) / 2;
    }

    template <typename T>
    static bool printTypewriter(T &lcd, const char* text, uint8_t col, uint8_t row, uint32_t speedMs, MTXTypewriter &state) {
      size_t len = strlen(text);
      if (state.currentIndex >= len) return true;
